Meteor.methods({
	addThought: function (text, question, location) {
        if(!UserLoggedIn) return false; // Make sure the user is logged in before inserting a thought
        //var friendList = Friends.findOne({userId:Meteor.userId()},{friendList:1,_id:0});
        var newThought = {
            text: text,
            question: question,
            createdAt: new Date(),
            userId: Meteor.userId(),
            rank: (Math.random() *.5),
            username: Meteor.user().username,
            position: location,
            collectedBy: [],
            randomIndex: Math.floor(Math.random() * 100000000) + 1,
            privacy: 'private'
        };
        var thoughtId = Thoughts.insert(newThought);
        collect();
        return thoughtId;
    },
    updatePrivacy : function(id, value, ownerId) {
        thought = Thoughts.findOne({_id: id});
        if (ownerId == thought.userId) {
            Thoughts.update({_id: id}, {$set: {privacy: value}});
        }
    },
    addSplashThought: function (text, location) {
        //if(!UserLoggedIn) return false; // Make sure the user is logged in before inserting a thought
        //var friendList = Friends.findOne({userId:Meteor.userId()},{friendList:1,_id:0});
        var newThought = {
            text: text,
            createdAt: new Date(),
            userId: "keepspace",
            rank: (Math.random() / 2),
            username: "keepspace",
            position: location,
            collectedBy: [],
            randomIndex: Math.floor(Math.random() * 100000000) + 1,
            privacy: 'splash'
        };
        var thoughtId = SplashThoughts.insert(newThought);
        //collect();
        return thoughtId;
    },

		addHiThought: function (date, text, question, location) {
        //if(!UserLoggedIn) return false; // Make sure the user is logged in before inserting a thought
        //var friendList = Friends.findOne({userId:Meteor.userId()},{friendList:1,_id:0});
        var newThought = {
            text: text,
						questionMark: question,
						dateNumber: date,
            createdAt: new Date(),
            userId: "keepspace",
            rank: (.2),
            username: "keepspace",
            position: location,
            collectedBy: [],
            randomIndex: Math.floor(Math.random() * 100000000) + 1,
            privacy: 'splash'
        };
        var thoughtId = HiThoughts.insert(newThought);
        //collect();
        return thoughtId;
    }


});
